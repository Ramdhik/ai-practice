...
export default function Home() {
  ...
  const renderMessage = (message: Message, index: number) => {
    ...
    return <div key={index} className={`p-2 rounded-lg ${message.role === 'user' ? 'bg-blue-100' : 'bg-green-100'}`} dangerouslySetInnerHTML={{ __html: formattedContent }} />;
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-2xl p-4 bg-white rounded-lg shadow-md">
        <div className="mb-4 space-y-2 overflow-auto h-96">{conversation.map((message, index) => renderMessage(message, index))}</div>
        <div className="flex">
          <input
            type="text"
            value={input}
